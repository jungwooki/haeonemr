...
const labels={indication:'주요 증상',summary:'치료 소개',tcm_story:'한의학적 설명',analogy:'쉽게 이해하기',scientific_evidence:'작용 설명',mechanism:'치료 원리',ingredients:'구성 약재',note:'복용 및 참고 안내'};
function chart(data){
 const names={qi_def:'기허',qi_stag:'기울',qi_counter:'기역',blood_def:'혈허',blood_stasis:'어혈',fluid:'수체',yin_def:'음허',phase_inflam:'어혈/염증기',phase_prolif:'재생/증식기',phase_remodel:'리모델링기',phase_maint:'유지/재활기'};
...
function render(){
 ...
 for(const item of items){const button=document.createElement('button');button.className='guide-card';button.innerHTML=`<span class="guide-tag">${esc(item.category)}</span><h2>${esc(item.name)}</h2><p class="guide-muted">${esc(item.summary||item.indication)}</p><p style="color:#2563EB;margin-top:20px;font-weight:700">자세히 보기 →</p>`;button.onclick=()=>openItem(item);grid.append(button);}
